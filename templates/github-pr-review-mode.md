...
Assemble the exact same `comments` array B2 would (`path`, `line`, `body` per finding, each anchored to the exact line number, each `body` opening with the B2 structured tag), plus the `anchor` field defined below — but do **not** issue the `gh api ... POST` call. Return the assembled array to the caller instead. B1's finding-selection step still applies as normal (or is satisfied by the caller the same way it is for B2). Skip B3 entirely in this variant — nothing has been posted yet, so there is no result to confirm here; the caller is responsible for confirming once it completes its own `POST`.

**`line` is the line number in the file as it exists at the PR head commit — never an offset into a diff, patch, or split-diff artifact you were handed.** An agent reviewing a `.diff`/`.patch` file must resolve each finding back to its source-file line before returning it: the hunk header `@@ -a,b +c,d @@` gives the base, or a bounded `grep -n '<anchor text>' <file>` finds it directly. This matters because the caller anchors GitHub comments on these values without re-deriving them.

**Every finding carries an `anchor`: the exact text of the line it points at, copied verbatim from the file.** One line, trimmed of leading/trailing whitespace, no ellipsis, no paraphrase. You already have the file open when you write the finding — copying the line costs nothing at that moment, and it is the only thing that lets the caller confirm the anchor without re-opening the file.

This exists because the caller otherwise cannot trust `line`, and re-derives it the expensive way. Measured across four real runs: every merge step independently invented a verification pass, re-reading 20–31 source files to check anchors it had no cheaper way to confirm — **13–35% of that step's entire token cost and 2.1–4.0 minutes of critical path**, and worse, the full-file reads inflated its context by 21k–102k tokens which then re-billed on every remaining turn. Two agents in that sample returned diff-relative offsets, which is what taught the callers not to trust the number.

A finding whose line genuinely cannot be resolved to a single source line returns `line: null` with the `anchor` text still populated, so the caller can locate it by search rather than guessing a number.
