...
description: >
  ...
  code-review / tests-code-review / complete-review directly).
metadata:
...
- Do NOT act on a review thread whose review is still pending/unsubmitted — comment-triage only processes **published** (submitted) review comments. The pending review Step 6 published (via `complete-review`) sits untouched until the user submits it.
...
- If a Step 8 or Comment-Triage subagent reports it could not complete (PR not found, auth failure, skill-invocation error, or — for a triage fix — a blocker it can't resolve): retry once with a fresh subagent. If it fails a second time, stop and report the failure — do not fabricate a finding count, skip a skill, mark a triage thread resolved on a failed fix, or fabricate a doc-sync outcome.
