...
**Before ending a turn at `code-review`'s checkpoint**, record `code_review: pending` in `progress.md` (`scripts/progress.mjs --step 11 --label "code-review, paused at checkpoint" --detail "review posted on PR #<N>" --set code_review=pending`), so an interruption resumes correctly — `pr_number` is already in Run State. **Resuming** with `code_review: pending` means the review is already posted: show the PR URL and wait for the user's reply again (never auto-approve because time passed), then invoke `code-review`'s continue-after-checkpoint entry ("continue the code review on PR #<N>") with the same fields Step 11's first invocation passes — PR number, owner/repo, feature folder path, and `worktree_path`, all from `progress.md` — so the fix worker runs in the existing worktree and writes `fix-code-review.md`. It submits the review and runs the fix stage, and never re-runs the review.
