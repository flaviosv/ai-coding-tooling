...
- **Recovery depends on what failed.** `code-review` retries each failure once itself: a failed review is re-run, a failed post is re-run from the same `post.json`, a failed submit or delivery goes through continue-after-checkpoint. That is the one retry per failure: if `code-review` still reports a failure, Step 11 stops and reports it, never retrying again. The continue entry is never used for a review or posting failure — it never posts, so using it there would mark a PR ready with its findings lost.
