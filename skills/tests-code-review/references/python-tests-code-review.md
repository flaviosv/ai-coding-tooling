...
# Good — session scope is appropriate for expensive read-only resources
@pytest.fixture(scope="session")
def db_engine():
...
### Python 3.9 (base) — Legacy `Optional` in Test Helper Signatures
...
### Python 3.11 — `ExceptionGroup` with `asyncio.TaskGroup`
...
class SpecificTestCase(BaseTestCase):
    def setup_fixutres(self) -> None:  # typo — no error, no override
...
# Good — itertools.batched() is clearer (Python 3.12+)
...
### Python 3.13 — `copy.replace()` for Test Variants
...
### Python 3.14 (latest stable) — Thread Safety in Free-Threaded Tests

When targeting free-threaded CPython (`-X gil=0`), test concurrent scenarios explicitly:
