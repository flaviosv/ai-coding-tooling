...
- Whichever context is already running this skill's GitHub Mode does that fixing work itself, inline, with no further nested `Agent` dispatch: `build-feature`'s own dispatched subagent (its fix-review step), or a Batch Mode per-PR subagent, already is the isolated context this used to try to recreate one level deeper. The one exception is a **live, direct invocation with no outer dispatch already isolating it** — a user invoking this skill in their own conversation, not through `build-feature` or Batch Mode. There, wrap the entire fixing pass (GitHub Mode step 5) in **one** Haiku subagent for the whole run, to keep the live conversation's own context clean — one subagent, not one per cluster. Follow [Subagent Dispatch Contract](../../templates/subagent-dispatch-contract.md) for that dispatch's shape: a completion condition tied to every surviving item having a fixed/rejected/blocked outcome, and a return shape carrying exactly that, plus commit SHAs — no further delegation from inside it.
- Before any reply, resolve, or push: run the validation gate (GitHub Mode step 6) confirming the actual composite state left behind once every item has landed together — not just that each item's own fix held individually at the moment it was made. And before this skill may report a thread as resolved: re-fetch its state from GitHub and confirm `isResolved: true` (GitHub Mode step 9) — never report a thread as fixed or resolved on the strength of having called the mutation alone. A real run committed 28 fixes, reported "40 threads fixed, 0 blocked," and pushed three times, without ever calling the reply or resolve mutation on GitHub — the report was true about the code and false about GitHub, because nothing checked its own claimed end-state before saying so.
