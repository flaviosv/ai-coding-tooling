...
5. Draft T1, T4, T6 concurrently (3 Haiku subagents), each including a test for its fix and a plain-language summary of what it changed. Apply + commit one at a time: `test(orders): cover rate-limit edge case` (T1), then T4's directed fix, then T6's fix — each running only its own relevant test first.
