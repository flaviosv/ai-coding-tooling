...
- **Replies and resolves are delivered by `scripts/deliver.py`, never by hand** (GitHub Mode step 8). The script replies in batches, re-fetches to confirm each reply landed exactly once, resolves only the threads whose reply it confirmed, re-fetches again to confirm `isResolved`, and prints a JSON result whose every count comes from those re-fetches rather than from what it sent. Step 10's report quotes that JSON verbatim — its numbers **are** the report's numbers, and there is no other route by which this skill may claim a thread was replied to or resolved. Never hand-roll the mutations, and never substitute `gh pr review` in any form, a REST reply endpoint, or a top-level PR comment: none of those is a threaded reply, and reaching for one means leaving the only delivery path this skill has. A non-zero exit means the run is **blocked** — report it with the script's raw JSON, never as a partial success. This is mechanical rather than instructed because five real runs reported delivery that GitHub never received; see [STATE.md](STATE.md) AD-001 and AD-006 for two of them.
...
- Every fix must be evaluated for test impact, not just coverage-added: write a new test if the change isn't already covered (follow this project's `tests` skill conventions for how to write it well); update an existing test whose assertions the fix invalidates; remove a test outright only when it was asserting the very behavior the fix corrects (e.g. it locked in the bug), and replace it with a test of the corrected behavior whenever the fix leaves anything worth covering. Coverage should move up or down only as a direct, stated consequence of the fix — never let it drift incidentally; a removal with nothing added back to replace it needs a one-line reason in the report. Then run only that fix's directly relevant test(s) to confirm it passes, per Test Execution Scope — including its stop rule: once that targeted run is green, the fix is verified, and nothing widens without a named risk. Do NOT invoke tlc-spec-driven's full Verifier or gate-check cycle for this — that's unnecessary weight for a fix-triage pass; a straightforward relevant-test run is the bar here.
...
- GitHub Mode: every GitHub write is batched — up to 10 aliased mutations per request for replies, and again for resolves — but you do not hand-build those batches; `scripts/deliver.py` does, along with the pacing and the abuse-detection backoff (see [GitHub Delivery Mechanics](references/github-delivery.md) for what it does and why). This skill never creates or submits a review of its own: GitHub wraps each accepted thread reply in a single-comment `COMMENTED` review by itself, exactly as it does for a reply typed in the web UI, and that is the whole of this skill's review footprint.
...
- **Every `Agent` launch this skill makes runs on Sonnet** — Batch Mode's per-PR runs and the single whole-run wrapper for a live, non-batch invocation alike. This holds in every mode: a batch sweep's per-PR orchestration is the same GitHub Mode flow a direct invocation runs, so there is no tier of this skill's work that warrants a different model. The tier is set by the *judgment* this skill exercises, not by its delivery: `scripts/deliver.py` makes the GitHub writes model-independent, but deciding whether a finding holds up, whether it contradicts a stated requirement, and which thread a reply belongs to are not mechanisable, and those are exactly where a lower tier was measured failing — see [STATE.md](STATE.md) AD-008. Load the `subagent-dispatch` skill for the alias-only naming rule and the rest of the shared dispatch facts.
- Both remaining dispatch sites (Batch Mode's per-PR runs, and the whole-run wrapper for a live invocation) follow the `subagent-dispatch` skill's contract: completion condition is every review thread this run touched having reached a fixed/rejected/blocked/answered outcome, **confirmed** on GitHub per step 9 — not just attempted; return shape is the same structured breakdown Step 5/Batch Mode Step 5 already specify (counts by outcome, commit SHAs, blockers); delegation depth is none — neither dispatch site may itself launch a further `Agent`.
...
- Load the `subagent-dispatch` skill's wait protocol before the first dispatch — see Batch Mode Step 5 below for this mode's reporting difference from the protocol's default.
...
- **GitHub Mode: a dirty working tree is not this mode's problem to clean up.** Never `git stash`, `git restore`, `git checkout --`, or `git clean` anything this run didn't itself create, and never target a path outside the checkout you were given — least of all the parent repository. A real run's first two actions were `cd <parent repo> && git stash` and `git restore .specs/features/<feature>/progress.md`, the second of which would have destroyed its caller's live `build-feature` run state; only the harness refused them. `progress.md` in particular belongs to whoever invoked you and is being written while you run — leave it exactly as found. If uncommitted changes genuinely block a commit, name the files in the report and stop.
- **When this run is working inside a worktree, resolve every file path from that worktree's root** (`pwd`), never from the repository's usual location. A read at the non-worktree path does not reliably fail: the main checkout holds the same files at the same relative paths on a different branch, so it can return real, plausible, *pre-feature* content that silently invalidates the "does this finding still hold?" judgment at step 5. One real run hit exactly this — two such reads failed loudly, a third succeeded and returned the wrong branch's file.
...
2. Resolve a PR number the same way `code-review` does: only from what's already established in this conversation (stated explicitly, or produced by an earlier step, e.g. `code-review` or `build-feature` opening/reviewing one). Do not infer it from git/gh branch state.
...
8. **Compose every reply, then hand delivery to the script.** Composing is yours; delivering is not. Each thread's reply text and whether it resolves come from its outcome: **fixed** (an auto-fix or apply-as-directed item completed at step 5) — a summary of what changed, why, and any test impact; resolve — never resolve without that reply, the thread is meant to read as a record of what was done. **rejected** (an item determined at step 5 not to hold up) — the rejection reasoning, and the approach taken instead if any; resolve. **blocked/unclear from step 5** — no reply unless there's a specific clarifying question; do not resolve. **answer-only** (decided at step 2) — the answer only; do not resolve, only the user resolves it once satisfied. **unclear** (decided at step 2) — a clarifying reply only if you have a specific question; do not resolve either way. **routed to a person** — no reply, no resolve, thread left completely untouched; it's the addressed person's turn, not this skill's.

   Write those into one delivery file, then run the script — these two plain commands, nothing else:

   ```
   # delivery.json — every in-scope thread appears in exactly one of the two maps
   ...
   ```
   ```bash
   python3 ~/.claude/skills/fix-review/scripts/deliver.py <path-to-delivery.json>
   ```

   Write `delivery.json` with the `Write` tool (never a shell heredoc — a worktree-isolated session refuses those). The script path above is correct exactly as written: `~/.claude/skills/fix-review` is a symlink to this skill and the OS follows it transparently. Do not `find` for the script, do not resolve the symlink first, and do not rewrite it as a repo-relative path like `skills/fix-review/scripts/deliver.py` — that only resolves when the working directory happens to be the `ai-coding-tooling` checkout, which it is not during a real run. If and only if that path does not exist, this skill is installed project-locally: use `.claude/skills/fix-review/scripts/deliver.py`. Those are the only two locations — never search for a third.

   **`threads` and `skipped` together must account for every in-scope thread on the PR** — in-scope meaning published (not on an unsubmitted review) and not already resolved. Every thread you are replying to goes in `threads`; every thread you are deliberately leaving alone goes in `skipped` with a real reason — routed-to-person, unclear, blocked, or an answer-only thread the user resolves themselves. The script fetches the PR's threads itself and reports any in-scope thread in neither map as `unaccounted`, exiting non-zero. That is the coverage check: it is what makes silently dropping a thread impossible rather than merely discouraged, after a real run fetched 43 threads, reported outcomes for 11, and left the other 32 unmentioned.

   The script batches the replies, re-fetches to confirm each landed exactly once, resolves only the threads whose reply it confirmed, re-fetches again to confirm `isResolved`, and handles GitHub's abuse-detection block on its own. Re-running it after a partial failure is safe: it skips any thread that already carries your reply, so nothing is duplicated. Pass `--dry-run` first to check your coverage before any write happens.
9. **Read the script's result — it is the run's end-state, not your recollection of it.** The JSON on stdout carries `in_scope`, `replied_confirmed`, `resolved_confirmed`, `unaccounted`, `reply_missing`, `resolve_not_confirmed`, `duplicates_found`, and `errors`, every one of them derived from a re-fetch of GitHub rather than from what was sent. Exit code 0 means every in-scope thread was accounted for and every intended reply and resolve is confirmed present; exit 1 means at least one is not; exit 2 means nothing was confirmed at all. On a non-zero exit the run is **blocked**: carry the raw JSON into step 10 and never fold an unconfirmed thread into a count that implies GitHub reflects it. A non-empty `unaccounted` means threads exist on the PR that this run neither answered nor consciously skipped — go back to step 2 and classify them rather than reporting around them.
10. If a worktree was created for this run, remove it per Before Starting (unless a blocker left state worth inspecting first — see there). Report: threads processed by outcome (fixed, rejected, answered, and including unclear, routed-to-person, and blocked items, with reasons) **taking every reply/resolve count from step 9's script output rather than from your own record of what you did**, commits pushed, the validation gate's outcome, any test additions/updates/removals with their reasons, and — if step 1 hit the page-size cap — a note that additional unresolved threads may exist beyond what was fetched. Cite commit SHAs exactly as a command printed them — the short form from `git log --oneline`, or the full form from `git rev-parse HEAD`/`git show`. Never expand a short SHA into a full one and never write a SHA that no tool output in this run actually contains: a real run reported four 40-character SHAs of which three were invented by padding out short hashes.
...
Agent
  ...
  prompt: |
    ...
    5. Apply every other GitHub Mode guardrail as normal: read each thread's full
       ...
       fix and nothing wider at fix time (Test Execution Scope), then the validation gate
       ...
       auto-fix/apply-as-directed thread, deliver every reply and resolve through
       `python3 ~/.claude/skills/fix-review/scripts/deliver.py <delivery.json>` (step 8
       — never hand-rolled mutations, never `gh pr review`, never a top-level PR
       comment) and report that script's own JSON counts rather than your recollection,
       skip any thread addressed to a specific person by name/handle rather than fixing
       it yourself, and report unclear items unfixed rather than guessed at.
...
6. Validation gate: this project's build/typecheck command plus the tests covering the four files this run touched — resolved from its `docs/codebase/STACK.md`/`TESTING.md`, not assumed — pass on the composite state; no conflict markers found (nothing to conflict — everything committed sequentially in one checkout). Note what this is *not*: a whole-suite run. "The tests covering every file this run touched" means those files' own tests, named explicitly; `./...`, `-A`, an unfiltered `pytest`, or a verbose flag all overshoot it.
7. Push (3 new commits) from the current working directory.
8. All six replies are composed — T1 ("Added a test covering the rate-limit edge case at the window boundary — previously untested."), T4 ("Extracted the rate-limit check into a shared helper as directed, with a test for the shared path."), T6 (its own fix summary), T2 ("already handled by a later commit on this branch"), T5 (the rejection reasoning), and T3 (the sliding-window tradeoff explanation) — and written to `delivery.json` with the `Write` tool: all six carry a `body`, and all but T3 carry `"resolve": true` (T3 is answer-only, so the user resolves it). Then one command: `python3 ~/.claude/skills/fix-review/scripts/deliver.py delivery.json`.
9. The script exits 0 and prints `"replied_confirmed": 6, "resolved_confirmed": 5, "reply_missing": [], "resolve_not_confirmed": [], "duplicates_found": []` — every count re-fetched from GitHub, not inferred from what was sent. Those are the numbers step 10 reports.
...
4. This is a live, direct invocation with no outer dispatch already isolating it, so the fixing pass wraps in one Sonnet subagent for the whole run (per Guardrails). Inline, in order: 3 findings still hold on reading the file → fix + test + commit each; the 4th is genuinely ambiguous about which of two valid approaches the user wants — reported unclear instead of guessed at. Validation gate (build + the three affected tests) passes on the composite state.
5. No push.
