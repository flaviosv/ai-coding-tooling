...
- Do NOT set `isolation: worktree` on the drafting or commit-application subagents — they must share whichever checkout is already in play (the user's own working directory in a direct GitHub Mode or Session-Only Mode run; the per-PR worktree a Batch Mode subagent is already running in — see Batch Mode below), so a fix lands ready for GitHub Mode's `git push`, or sits ready in the working tree for Session-Only Mode's local commits. This is distinct from Batch Mode's own outer isolation (per PR, not per fix) — see Batch Mode's guardrails.
...
- Each qualifying PR's fix run happens in its own subagent, in its own isolated git worktree (`Agent` tool, `agentType: general-purpose`, `model: sonnet`, `run_in_background: true`, `isolation: 'worktree'`) — this is what lets every qualifying PR fix concurrently without fighting over which branch is checked out in a shared working directory. The subagent's first action inside its worktree is to check out that PR's branch (`gh pr checkout <N>`); this checkout is only safe because the worktree isolates it from the user's own checkout and from every other PR's subagent — a direct (non-batch) GitHub Mode run never auto-checks-out a branch for exactly this reason (see Before Starting). Launch every subagent's `Agent` call in the same message/turn — never one at a time.
...
- GitHub Mode, direct invocation only (not a Batch Mode subagent — those already run isolated, see Batch Mode): the working directory must already be checked out to the PR's head branch — compare `git branch --show-current` against `gh pr view <PR> --json headRefName --jq .headRefName`. If they don't match, stop and ask the user to check out the PR's branch first. This mode shares the user's own working directory, so it never switches branches out from under them.
...
8. Report: threads processed by classification (including unclear, routed-to-person, and blocked items, with reasons), commits pushed, any test additions/updates/removals with their reasons, and — if step 1 hit the page-size cap — a note that additional unresolved threads may exist beyond what was fetched.
...
5. Draft T1, T4, T6 concurrently (3 Sonnet subagents), each including a test for its fix and a plain-language summary of what it changed. Apply + commit one at a time: `test(orders): cover rate-limit edge case` (T1), then T4's directed fix, then T6's fix — each running only its own relevant test first.
