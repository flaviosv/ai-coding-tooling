...
- GitHub Mode: a run posts **exactly one** review, holding every reply it makes — never one review per reply. Replying without a `pullRequestReviewId` makes GitHub auto-submit a fresh single-comment review per call, which is what leaves a PR with a stack of near-simultaneous "reviewed" entries; batch them into one pending review and submit that once instead (see [GitHub Delivery Mechanics](references/github-delivery.md)). Submit with `event: COMMENT` only — never `APPROVE` or `REQUEST_CHANGES`.
...
7. No pending review of your own exists on #128 → create one, and post all six replies into it: T1 ("Added a test covering the rate-limit edge case at the window boundary — previously untested."), T4 ("Extracted the rate-limit check into a shared helper as directed, with a test for the shared path."), T6 (its own fix summary), T2 (its subagent's "already handled by a later commit" reasoning), T5 (its subagent's rejection reasoning), and T3 (the sliding-window tradeoff explanation). Submit that one review with `event: COMMENT` — one "reviewed" entry on the PR, not six — then resolve T1, T2, T4, T5, and T6. T3 stays unresolved.
