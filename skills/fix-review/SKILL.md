...
- As soon as a PR's completion notification arrives — success or failure, right after reporting it — remove that PR's own worktree (`git worktree remove <path>`; the path was recorded when the subagent was launched). Unlike `build-feature`'s deliberately long-lived worktrees (kept open across runs to support later re-entry), a Batch Mode worktree exists only for this one fix run and has nothing to resume later — leaving it on disk after its result is reported is a leak, not a feature. This is in addition to, not instead of, each subagent's own per-cluster worktree cleanup inside its run (see Scope guardrails).
