...
- **Every `Agent` launch this skill makes runs on Sonnet** — Batch Mode's per-PR runs and the single whole-run wrapper for a live, non-batch invocation alike, per [Subagent Models](../../templates/subagent-models.md). This holds in every mode: a batch sweep's per-PR orchestration is the same GitHub Mode flow a direct invocation runs, so there is no tier of this skill's work that warrants a different model. The tier is set by the *judgment* this skill exercises, not by its delivery: `scripts/deliver.py` makes the GitHub writes model-independent, but deciding whether a finding holds up, whether it contradicts a stated requirement, and which thread a reply belongs to are not mechanisable, and those are exactly where a lower tier was measured failing — see [STATE.md](STATE.md) AD-008. Set `model` explicitly on each launch and pass it as one of the tool's short aliases *verbatim* — `sonnet`, `opus`, `haiku`, `fable`. Never resolve an alias into a versioned model ID (`claude-haiku-4-5-…`, `claude-sonnet-5`, …) on the way to the call, however confidently the environment advertises one: the param accepts only the four aliases, so a versioned ID fails input validation and the subagent never starts. A launch rejected that way did nothing at all — no worktree, no checkout, no commit — so correct the param and relaunch it; it doesn't consume the one retry below.
