...
- Do NOT trust a remembered list of what was posted earlier in this conversation — GitHub is the source of truth. Always fetch review threads fresh (GraphQL `reviewThreads`) before acting; a manual comment may have been added since anything was last posted, and this must be picked up. This cuts both ways: in GitHub Mode, fix only threads/comments that actually exist on GitHub right now — never fold in a finding that was only ever mentioned in this conversation (e.g. from an earlier `code-review` run that was never posted) just because it's about the same PR. Conversation history is used only to resolve which PR to act on (Step 1) or, in Session-Only Mode, as the findings themselves — never as an extra source of findings once GitHub Mode is running.
...
- Do NOT implement a fix directly in this conversation — delegate each item to a single Haiku subagent that drafts, applies, and commits the fix in one call, one item at a time (never two concurrently — each ends in a commit on the shared checkout, and concurrent commits would race); keep only classification and reply/reject reasoning here.
- Do NOT set `isolation: worktree` on these per-item subagents — they must operate inside whichever checkout is already in play: the isolated worktree direct GitHub Mode creates for itself before Step 1 when it isn't already on the PR's branch, or the current working directory when it already was (per Before Starting); the per-PR worktree a Batch Mode subagent is already running in (see Batch Mode below); or, for Session-Only Mode, the user's own working directory. Pass that location's path explicitly in every per-item subagent's prompt, so a fix lands ready for GitHub Mode's `git push` from the right place, or sits ready in the working tree for Session-Only Mode's local commits. This is distinct from Batch Mode's own outer isolation (per PR, not per fix) — see Batch Mode's guardrails.
...
- If a per-item subagent reports it could not complete (a blocker it can't resolve): retry once with a fresh subagent. If it fails a second time, stop and report the failure — do not fabricate a result or mark an item resolved on a failed fix.
...
5. For the auto-fix and apply-as-directed items surviving step 4: fix them one at a time, in the plan's order (`## Parallel` items then `## Sequential` items) — one subagent per item (`Agent` tool, `agentType: general-purpose`, `model: haiku`, `run_in_background: false`) that drafts, applies, and commits the fix in a single call; never run two of these concurrently, since each ends in a commit on the shared checkout and concurrent commits would race. Each subagent uses the PR's own title/body/diff for context, is told the checkout location resolved in Before Starting (the isolated worktree, or the current working directory if none was needed), reads the relevant files there, makes the edit, follows Conventional Commits for the commit — including any test it wrote, updated, or removed to keep coverage aligned with the fix, with a one-line reason for any removal — and runs only that item's own directly relevant test(s) to validate before committing (no full gate/verify cycle). It returns a few-sentence plain-language summary of what the fix actually does and why, for use as the thread's documentation reply (step 6) — or a blocker/unclear reason if it can't confidently complete, in which case it makes no edit and no commit. If the file state no longer matches what step 4's re-fetch assumed by the time the subagent gets to it, mark the item blocked instead of force-applying. For answer-only, pushback, routed-to-person, or unclear items: no commit, no subagent — compose the reply (or the "here's what's unclear" note) directly in this conversation, on the default model.
...
4. Fix each finding with the same mechanics as GitHub Mode step 5 (one Haiku subagent per item, draft+apply+commit combined in a single call, one item at a time, Conventional Commits, test coverage required, unclear items reported unfixed rather than guessed at) — minus anything about threads, since there are none here.
...
Agent
  ...
  prompt: |
    ...
    2. Invoke the `fix-review` skill via the Skill tool, GitHub Mode, targeting PR #<N>
       specifically — do not rely on any prior conversation context, there is none.
    3. When fetching and classifying review threads (GitHub Mode steps 1-2), restrict
    ...
    4. Apply every other GitHub Mode guardrail as normal: read each thread's full
       ...
       thread, commit one item at a time, skip any thread addressed to a specific
       person by name/handle rather than fixing it yourself, and report unclear items
...
`ship-spec`'s Comment-Triage Mode invokes `fix-review` for PR #128, stating the active feature (`PROJ-42_rate-limiting`). The working directory is already on PR #128's branch — `ship-spec` just delivered it from here.
...
4. Feature is active → write `fix-code-review.md`: `## Parallel` — T1, T4, T6 (no two share a file); `## Sequential` — none. T2, T3, T5 listed for audit, excluded from drafting.
5. Re-fetch: nothing changed — all 6 items proceed
6. Fix T1, T4, T6 one at a time (3 Haiku subagents, draft+apply+commit combined in each call), every subagent including a test for its fix and a plain-language summary of what it changed: `test(orders): cover rate-limit edge case` (T1), then T4's directed fix, then T6's fix — each running only its own relevant test first.
...
4. Fix 3 of them one at a time (Haiku subagents, draft+apply+commit combined in each call, each with its own test); the 4th is genuinely ambiguous about which of two valid approaches the user wants — reported unclear instead of guessed at. 3 commits made, each with its relevant test passing.
5. No push.
...
2. It's recorded for the report (thread id, `path:line`, addressed to `@bob`) but excluded from the `## Parallel`/`## Sequential` plan — no drafting subagent, no reply, no resolve.
