...
- Do NOT implement a fix directly in this conversation — delegate fixes to Haiku subagents, one subagent per **file cluster**: every auto-fix/apply-as-directed item whose fix would touch a given file belongs to that file's cluster, and one subagent handles its whole cluster, working through its items strictly one at a time inside that single call (edit → that item's relevant test → commit → next item) — never split one cluster's items across two subagents, and never two subagents against the same cluster. Keep only classification, clustering, and reply/reject reasoning here.
- Every per-cluster subagent MUST run with `isolation: 'worktree'` — file-disjointness between clusters rules out merge conflicts, but it does not make concurrent `git add`/`git commit` calls against one shared checkout safe: the index and working tree are shared by the whole checkout, not scoped per file, so two clusters committing at the same moment in the same checkout can race and bundle each other's changes into the wrong commit. Isolation removes the race instead of hoping it doesn't happen. Inside its own worktree, a cluster subagent fetches the PR's branch and checks out a **new local branch name of its own** (e.g. `fix-review-cluster-<n>`) pointed at that branch's tip — never the PR's own branch name, which is already checked out in whichever checkout Before Starting resolved (the base checkout) and would collide. It commits there exactly as before, one item at a time, and never pushes and never touches the base checkout directly. Once every cluster subagent for this run has returned, merge their work into the base checkout before continuing to step 6: for each cluster that produced at least one commit, in any order (disjoint files guarantee no conflicts) — fetch that cluster's worktree/branch into the base checkout and `git cherry-pick` its commit range onto the base checkout's branch there, preserving each item's own commit — then remove that cluster's worktree (`git worktree remove`). A cluster that produced zero commits (every item blocked/unclear) needs no merge, just its worktree removed. This is distinct from Batch Mode's own outer isolation (per PR, not per cluster) — see Batch Mode's guardrails.
...
- **Every `Agent` launch this skill makes runs on Haiku** — Batch Mode's per-PR runs and GitHub/Session-Only Mode's per-cluster fix runs alike, per [Subagent Models](../../templates/subagent-models.md). This holds in every mode: a batch sweep's per-PR orchestration is the same GitHub Mode flow a direct invocation runs, so there is no tier of this skill's work that warrants a different model. Set `model` explicitly on each launch and pass it as one of the tool's short aliases *verbatim* — `sonnet`, `opus`, `haiku`, `fable`. Never resolve an alias into a versioned model ID (`claude-haiku-4-5-…`, `claude-sonnet-5`, …) on the way to the call, however confidently the environment advertises one: the param accepts only the four aliases, so a versioned ID fails input validation and the subagent never starts. A launch rejected that way did nothing at all — no worktree, no checkout, no commit — so correct the param and relaunch it; it doesn't consume the one retry below.
- If a cluster subagent fails outright (crashes, errors, can't proceed at all — distinct from marking one of its own items blocked/unclear and continuing on to the rest, which needs no retry): remove its worktree (nothing to merge from a failed attempt) and retry once with a fresh subagent and a fresh worktree for that cluster. If it fails a second time, remove that worktree too, then stop and report the failure — do not fabricate a result or mark an item resolved on a failed fix.
...
- As soon as a PR's completion notification arrives — success or failure, right after reporting it — remove that PR's own worktree (`git worktree remove <path>`; the path was recorded when the subagent was launched). A Batch Mode worktree exists only for this one fix run and has nothing to resume later — leaving it on disk after its result is reported is a leak, not a feature. This is in addition to, not instead of, each subagent's own per-cluster worktree cleanup inside its run (see Scope guardrails).
...
5. For the clusters surviving step 4: launch one subagent per cluster, all in the same message/turn (`Agent` tool, `agentType: general-purpose`, `model: 'haiku'` — the literal alias, never a versioned model ID, see Guardrails — `isolation: 'worktree'`, `run_in_background: true` when dispatching more than one cluster this run, otherwise `false`) — clusters run concurrently, each in its own isolated worktree (see Guardrails for why disjoint files alone don't make a shared checkout safe, and the fetch/checkout-new-branch/cherry-pick-back mechanics). A cluster with more than one item is handled entirely by that one subagent, which works through its items strictly one at a time inside the call — never split across subagents, never parallelized within the cluster, since each item ends in its own commit and a cluster's own items still share its one worktree. Each subagent uses the PR's own title/body/diff for context, is told the PR's head branch and the base checkout's path (for the fetch it does inside its own worktree, per Guardrails), and is given its full list of assigned items (thread id, classification, `path:line`, and what the thread is actually asking for). For each item in order, the subagent first reads that item's target file(s) — the read it needs anyway to make the edit — and judges whether the underlying finding or suggested approach still holds against what's actually there now (see Guardrails: never fix a finding just because it exists). This is the only place that judgment happens, once, by whoever is already reading the code. If it holds: makes the edit, follows Conventional Commits for that item's own commit — including any test it wrote, updated, or removed to keep coverage aligned with that fix, with a one-line reason for any removal — and runs only that item's own directly relevant test(s) to validate before committing it (no full gate/verify cycle — the subagent's prompt must name that scope explicitly, per [Test Execution Scope](../../templates/test-execution-scope.md), never leaving verification unqualified). If it doesn't (stale, based on a misread of the diff, already addressed, or simply wrong): no edit, no commit — the item comes back **rejected**, with the subagent's reasoning for why and, if relevant, the approach it would take instead. Either way, move to the cluster's next item. It returns, per item, one of: a fixed summary (what changed and why, for that item's thread reply), a rejected reason, or a blocker/unclear reason for any item it can't confidently complete — in which case that item gets no edit and no commit but the subagent still continues on to the rest of its cluster. If the file state no longer matches what step 4's re-fetch assumed by the time the subagent reaches a given item, mark that item blocked instead of force-applying — this doesn't stop the rest of the cluster. Once every dispatched cluster subagent has returned, cherry-pick each cluster's commits into the base checkout and remove its worktree, per the Guardrails mechanics, before continuing to step 6. For answer-only, routed-to-person, or unclear items (decided already in step 2): no commit, no subagent — compose the reply (or the "here's what's unclear" note) directly in this conversation, on the default model.
6. Compose every reply this run makes, post them in batches, verify they landed, then resolve in batches — never one request per thread (see [GitHub Delivery Mechanics](references/github-delivery.md): 10 aliased mutations per request, replies first, then one query confirming each thread carries exactly one reply from you, then resolves for the threads that pass it — a failed-looking reply request may well have landed, so nothing is ever retried without that check). Each thread's reply and whether it resolves come from its outcome: **fixed** (an auto-fix or apply-as-directed item its subagent completed) — reply with the subagent's fix summary (what changed, why, and any test impact), then resolve — never resolve without that reply, the thread is meant to read as a record of what was done. **rejected** (an auto-fix or apply-as-directed item its subagent determined doesn't hold up) — reply with the subagent's rejection reasoning (and the approach taken instead, if any), then resolve. **blocked/unclear from step 5** — no reply unless there's a specific clarifying question, leave unresolved, same as step 2's **unclear** below. **answer-only** (decided at step 2) — reply with the answer only; leave unresolved, only the user resolves it once satisfied. **unclear** (decided at step 2) — post a clarifying reply only if you have a specific question; leave unresolved either way. **routed to a person** — do not reply, do not resolve; leave the thread completely untouched, it's the addressed person's turn, not this skill's.
...
8. If a worktree was created for this run, remove it per Before Starting (unless a blocker left state worth inspecting first — see there). Report: threads processed by outcome (fixed, rejected, answered, and including unclear, routed-to-person, and blocked items, with reasons), commits pushed, any test additions/updates/removals with their reasons, and — if step 1 hit the page-size cap — a note that additional unresolved threads may exist beyond what was fetched.
...
3. Take every finding already known from this conversation — their presence in the conversation is the go-ahead to evaluate and act, same as a published GitHub review comment is in GitHub Mode, not a mandate to apply them unread. Group and dispatch them as in step 4 below; whether each one actually holds up against the current code is confirmed once, by its assigned subagent, at fix time (see Guardrails) — not as a separate pass here. One that doesn't hold up (stale, already addressed, or simply wrong) comes back rejected with the reason instead of fixed.
4. Fix each finding with the same mechanics as GitHub Mode step 5 (group into file clusters, one Haiku subagent per cluster in its own isolated worktree, each item confirmed against the current code and either fixed or rejected at that same read, processed one at a time within a single call, clusters dispatched concurrently, each cluster's commits cherry-picked back into the working directory once it returns, Conventional Commits, test coverage required, verification scoped and stated per [Test Execution Scope](../../templates/test-execution-scope.md), unclear items reported unfixed rather than guessed at) — minus anything about threads, since there are none here.
...
Agent
  ...
  prompt: |
    ...
    5. Apply every other GitHub Mode guardrail as normal: read each thread's full
       ...
       and nothing wider (Test Execution Scope, ../../templates/test-execution-scope.md),
       ...
       cluster in its own isolated worktree (each cluster's own items committed one at
       a time within its subagent, then cherry-picked back into this worktree once it
       returns), skip any thread addressed to a specific person by name/handle rather
       than fixing it yourself, and report unclear items unfixed rather than guessed
       at.
...
4. Feature is active → write `fix-code-review.md`: five single-item clusters — T1, T2, T4, T5, T6 (no two share a file) — plus T3 listed for its answer-only reply, excluded from dispatch.
5. Re-fetch: nothing changed — all 5 items proceed
6. Dispatch 5 concurrent Haiku subagents (one per cluster, each in its own isolated worktree, combining confirm+draft+apply+commit in one call). T1, T4, and T6's subagents confirm their finding/direction still holds on reading the file, then fix+test+commit: `test(orders): cover rate-limit edge case` (T1), T4's directed fix, and T6's fix — each running only its own relevant test before committing. T2's subagent finds, on reading the file, that it's already handled by a later commit on the same branch → **rejected**, no edit. T5's subagent finds the suggested approach doesn't hold up against what's actually there → **rejected**, no edit, with its reasoning. Cherry-pick T1/T4/T6's commits into the base checkout; T2 and T5 produced none.
7. All six replies go out in **one** batched request — T1 ("Added a test covering the rate-limit edge case at the window boundary — previously untested."), T4 ("Extracted the rate-limit check into a shared helper as directed, with a test for the shared path."), T6 (its own fix summary), T2 (its subagent's "already handled by a later commit" reasoning), T5 (its subagent's rejection reasoning), and T3 (the sliding-window tradeoff explanation). All land, so a second batched request resolves T1, T2, T4, T5, and T6. T3 stays unresolved.
...
5. Batch Mode Step 4: two `Agent` calls launched in one message, one per PR, both `model: haiku`, `run_in_background: true`, `isolation: worktree` — each in its own isolated worktree, checking out its own PR's branch (`gh pr checkout 31` / `gh pr checkout 33`) without touching the other's or the user's own checkout, then running GitHub Mode scoped to only the user's own comments
...
5. GitHub Mode Steps 5–7 run as normal: both auto-fix items' subagents confirm the finding still holds, fix with tests, and commit; the apply-as-directed item's subagent finds the suggested exponential backoff unnecessary — the fixed linear cap already meets the requirement — and comes back rejected with that reasoning; 2 commits pushed.
