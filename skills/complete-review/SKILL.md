...
3. **Append new findings in batches of 5, one GraphQL request per batch, paced at least 1 second apart between batches** (GitHub's own documented remedy for bulk POST/PATCH/PUT/DELETE calls — applied per request, not per comment, now that a request can carry several). 5 is a **starting point, not a tuned constant** — the empirical REST data only tells us ≤20 comments in one payload was safe and 80 wasn't; nobody has tested where the line sits for a batched GraphQL request specifically, so start conservative. If the caller (or the user, after watching a run) says to try a larger batch size, use that instead — do not silently revert to 5 or auto-tune it down after a failure.

   Alias one `addPullRequestReviewThread` call per finding in the batch, all in a single mutation document — build the query string with exactly as many aliases as the batch has (2–5 near the end, never padded):
   ```
   gh api graphql -f query='
     mutation(
       $reviewId: ID!,
