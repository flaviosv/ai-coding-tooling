...
| `build-feature` | Step 5a — Specify | `sonnet` |
| `build-feature` | Step 5b — Design | `sonnet` |
| `build-feature` | Step 6 — Tasks | `haiku` |
| `build-feature` | Step 8 — Execute | `sonnet` |
| `build-feature` | Step 11 — architecture-evaluate | `sonnet` |
| `build-feature` | Step 13 — merge-conflict resolution | `sonnet` |
...
- **A tier change here is a pipeline change.** `code-review`'s review worker delegates to its dimension subagents and its fix worker acts on their findings, and `build-feature`'s Step 10 runs all of it; retiering one row without the others leaves the stack inconsistent. Change the rows together.
