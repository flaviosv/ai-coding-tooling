...
Then determine the **currently supported version range** using Context7 MCP (preferred) or your own knowledge as fallback:
...
2. From the results, identify which versions are currently receiving **active support or security updates** from their official maintainers.
...
For each skill selected by the user in Step 3, check whether a reference file already exists for the requested technology — including partial matches (e.g. a `python-*.md` file when adding `fastapi`, or a `go-*.md` when adding `go-gin`).
...
Before generating any reference files, use Context7 MCP (if available) to gather authoritative, up-to-date documentation. This step grounds reference files in official docs rather than relying solely on training data.
...
Reference files MUST be organized by version when the technology has multiple versions with meaningful differences. The version range is determined in Step 1 (oldest still-supported version → latest stable).
