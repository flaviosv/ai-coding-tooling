...
Per-test alternative when `TestMain` is not used:
...
Use `gin.CreateTestContext` when you need to inject context values directly without running middleware (e.g. `user_id` from auth middleware):
...
When you need to verify the full routing stack — path matching, middleware chain, and handler — use the same `setupRouter` function that `main.go` uses:
...
MUST verify the response body shape and key fields, not just status code:
