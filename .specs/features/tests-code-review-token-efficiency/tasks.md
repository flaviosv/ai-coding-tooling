...
**Done when:**
- [x] Step 2 instructs checking presence/absence only — no content loading from `references/` or `.specs/codebase/`
- [x] `docs/TECH_DEBTS.md` row and `tech_debts` key removed from Step 2
- [x] Step 3 availability map has exactly 9 keys (`architecture`, `concerns`, `conventions`, `integrations`, `stack`, `structure`, `testing`, `checklist_baseline`, `checklist_tech_specific`) — no `tech_debts`
- [x] Step 3 states orchestrator holds map only; agents self-load via `## Before You Begin` in Step 6
- [x] Bundle-assembly language ("context bundle", "structured text block injected") removed from Step 3
...
**Done when:**
- [x] EXCLUDE constant defined before the mode table — containing the full lockfile list, minified/built/generated, AND test-specific noise (`__snapshots__/**`, `*.snap`, `coverage/**`, `.nyc_output/**`)
- [x] Every `git diff`/`git show` command in the test-diff table applies `-- $EXCLUDE`
- [x] GitHub PR mode explicitly filters excluded paths from the changed-file list
- [x] `impl_diff` collection section applies EXCLUDE (not just a filter instruction)
- [x] Step 4 captures `excluded_count` for use in report header
- [x] EXCLUDE referenced once — not duplicated per mode
...
**Done when:**
- [x] "Quick Mode Check" heading and binary logic replaced by "Review Complexity Assessment"
- [x] Size-tier table present with all 4 tiers, exact thresholds, and "top-down, first match wins" rule: Small (≤5 OR <200 → inline), Medium (≤15 AND <800 → single agent), Large (≤25 AND <1,500 → parallel), Complex (>25 OR ≥1,500 → parallel + caveat)
- [x] Review Plan block defined (tier, mode, dimensions, agent count, gap-detector status, Complex flag, excluded count)
- [x] Complexity banner specified (prints before any dispatch/inline review, all modes/tiers, with exact format)
- [x] Silent-operation rule stated: only 3 user-facing outputs (skill invocation → banner → final report)
- [x] Medium tier gap-detector fold described: receives both diffs when `impl_diff` non-empty
...
**Done when:**
- [x] Opening dispatch rule describes 4 execution modes and which to use per size tier
- [x] Inline mode (Small): orchestrator applies all dimensions directly; gap-detector inline if `impl_diff` non-empty
- [x] Single-agent mode (Medium): ONE subagent, all dimensions; receives test diff + impl_diff if non-empty; agent self-loads union of checklists + full 7-doc codebase set
- [x] Parallel mode (Large/Complex): one agent per active dimension; Complex adds thoroughness directive
- [x] `## Before You Begin` block in prompt template: checklists (`test-review-checklist.md` + tech-specific if present) + full 7-doc codebase set — all filtered to present files
- [x] gap-detector's `## Before You Begin` lists only `STACK.md`, `ARCHITECTURE.md`, `CONCERNS.md`, `INTEGRATIONS.md`; no checklist
- [x] Return format has NO `Files reviewed: [list]` — findings only
- [x] "Tech Debt Recurrence" injection block removed
- [x] Agent Roster updated to reflect self-loading (Required/Optional context columns simplified to "Self-loads" description)
...
**Done when:**
- [x] Report header Diff line shows excluded count: `N files changed (M excluded as snapshots/lockfiles)` — when `excluded_count > 0`
- [x] Complex completeness caveat appears in report header when tier = Complex
- [x] Silent-operation rule stated at top of Step 8: exactly 3 user-facing outputs; no intermediate narration
...
**Done when:**
- [x] Each example includes: complexity assessment step → banner → correct execution mode → report
- [x] No example references old step numbering or "Quick Mode Check" language
- [x] Guardrails / What NOT to Review mentions EXCLUDE constant for noise files
- [x] `metadata.version` bumped to `2.2.0` (superseded by subsequent releases; current 2.4.1 after gh-CLI support, Sonnet-pin guardrail, and this correction)
