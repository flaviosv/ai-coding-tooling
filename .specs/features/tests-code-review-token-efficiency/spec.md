...
- [x] Add a complexity-assessment step that routes each review by size tier (Small/Medium/Large/Complex → inline / single-agent / parallel / parallel+caveat).
- [x] Use a single-agent execution mode for Medium reviews (1× diff) instead of parallel dispatch — the primary token win for the common case.
- [x] Exclude test-specific noise files (snapshots, coverage reports) from the diff at the `git diff` level so they never enter any context.
- [x] Remove checklist AND codebase-doc loading from the orchestrator; agents self-load only what their dimension needs.
- [x] Remove `docs/TECH_DEBTS.md` from Step 2 and the availability map (consistent with code-review's CR-TOKEN decision).
- [x] Strip `Files reviewed` roll-call from agent return format.
- [x] Preserve full review quality for all review modes — no regressions in findings coverage or output format.
...
- [x] Snapshot/coverage noise files excluded from diff at `git diff` level — verified by reviewing a PR with a large snapshot update and confirming snapshot content is absent from agent prompts.
- [x] Report header notes the count of excluded files (transparent, not silent drop).
- [x] Complexity-assessment step emits an explicit Review Plan (tier + mode + dimensions + agent count + gap-detector status) before any dispatch.
- [x] Complexity banner is printed at the start of every review (all tiers, all modes).
- [x] A 4-file/80-line test change is classified Small and reviewed inline (0 agents).
- [x] A 10-file/500-line test change is classified Medium and runs exactly ONE agent covering all dimensions (1× diff).
- [x] A 20-file/900-line test change is classified Large and runs parallel agents.
- [x] A 30-file/2,000-line test change is classified Complex: parallel agents + completeness caveat in header + thoroughness directive in each agent.
- [x] Medium tier with non-empty `impl_diff`: single agent receives both diffs, gap-detector dimension covered.
- [x] The skill emits only three user-facing outputs (skill invocation → banner → final report) with nothing in between.
- [x] Agent returns carry findings only (no `Files reviewed` roll-call); actual findings retain full detail in the final report.
- [x] Orchestrator Step 2 holds only an availability map — no file content from `references/` or `.specs/codebase/`.
- [x] Every reviewing agent (all except gap-detector) loads the full 7-doc codebase set including `TESTING.md`.
- [x] `gap-detector` prompt loads only `STACK.md`, `ARCHITECTURE.md`, `CONCERNS.md`, `INTEGRATIONS.md` and no checklist.
- [x] `docs/TECH_DEBTS.md` is loaded by nothing — grepping the skill for `TECH_DEBTS`/`tech_debts` returns zero matches.
- [x] Availability map has exactly 9 keys (no `tech_debts`).
- [x] Output format (at-a-glance table, zoned findings, finding IDs, severity/priority/type labels) is unchanged.
