...
### T3: Rewrite `ship-spec` Step 6 — concurrent analysis, merge, single post

**What**: Replace the current single-sequential-wrapper Step 6 with: two subagents dispatched concurrently (one per skill, Return-Only Variant, `model: sonnet`, `run_in_background: false`, issued in the same turn), each returning a `comments` array + counts (or a failure reason); merge both arrays; issue exactly one `POST .../reviews` call. Update the Guardrails bullets that describe the old mechanism (L29–30 delegation wording, L38 "never in parallel" — now inverted since GitHub's one-pending-review-per-PR limit is the real constraint being respected, not dispatch order; L39 partial-failure retry rule — adapt to "retry only the failed analysis subagent, then merge/post with whatever succeeded").
