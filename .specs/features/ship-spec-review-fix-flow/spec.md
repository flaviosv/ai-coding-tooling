...
| GitHub API concurrency risk from this feature | None — the two concurrent tool calls inside Step 6's one subagent do no GitHub writes at all (analysis only, Return-Only Variant); the single merged pending-review POST is the only GitHub write in Step 6, issued once, inside that same subagent, after both finish. Fix-drafting agents (up to 4) also do no GitHub calls under split-phase; commit/resolve/reply calls remain serialized | Superseded by the one-pending-review-per-PR constraint below — removes the concurrency-risk question entirely rather than mitigating it | n (derived from confirmed decision) |
| Step 6 publish mechanism, given GitHub allows only one pending review per user per PR at a time (HTTP 422 on a second concurrent/sequential attempt — confirmed via GitHub REST API docs and community reports) | One subagent (unchanged from today's dispatch shape), whose own turn runs both skills' analysis concurrently (no GitHub writes) via two tool calls instead of sequential awaits; findings are merged into one `comments` array and posted as a single pending review in one `POST .../reviews` call — entirely inside that subagent, so `ship-spec`'s own context still only ever sees a compact summary | The only mechanism GitHub's API actually supports for two skills contributing line-anchored findings to one PR without auto-submitting; refined during Execute from an initial two-top-level-subagents design once it became clear that design would leak full findings text into `ship-spec`'s persistent context — one subagent with internal concurrency is strictly better (same parallelism, one cold start not two, zero context leakage). Also flags that the currently-shipped Step 6 (claiming two separate pending reviews) doesn't match documented GitHub behavior and should be checked against real usage independent of this feature | y (confirmed after web verification + AskUserQuestion; internal-concurrency refinement applied during Execute per coding-principles.md, documented as SPEC_DEVIATION in tasks.md T3) |
...
1. WHEN Step 6 begins THEN `ship-spec` SHALL dispatch exactly one isolated subagent (as today), whose own turn SHALL issue two concurrent tool calls — one invoking `code-review`'s Return-Only Variant analysis, one invoking `tests-code-review`'s — rather than sequential awaits, with **neither invocation posting anything to GitHub itself**.
2. WHEN both invocations have returned their findings (within the subagent's own conversation) THEN the subagent SHALL merge both skills' findings into a single `comments` array and issue exactly one `POST .../pulls/{PR}/reviews` call (event field omitted, landing PENDING) covering both skills' findings in one pending review — entirely inside its own context.
3. WHEN the subagent has posted THEN it SHALL return **only** a compact summary (finding counts per skill, PR confirmation) to `ship-spec`'s own conversation — never the full findings text or comments array, preserving the existing context-isolation guarantee.
4. WHEN one of the two invocations fails before returning findings THEN the subagent SHALL post a single pending review containing only the succeeded skill's findings, and report which skill failed and why as part of its compact summary (never attempt a second separate pending-review call while the first is still pending).
5. WHEN the subagent's compact summary is returned THEN `ship-spec` SHALL proceed to Step 7 (report PR URL + finding counts) and stop, waiting for the user — unchanged from today's behavior.

**Independent Test**: Run `ship-spec` through Step 6 on a real PR with real findings in both dimensions; confirm exactly one pending review appears on GitHub containing line-anchored comments from both skills, that the two analysis invocations visibly overlap in time inside the subagent's own transcript (not one fully finishing before the other starts), and that `ship-spec`'s own conversation only ever sees the compact summary, never the raw findings.
