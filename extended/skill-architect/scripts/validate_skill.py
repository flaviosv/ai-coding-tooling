...
This is the skill-architect overlay's copy of the parent skill's validate_skill.py: every
parent check, CLI flag, and exit code is unchanged, plus two added error checks,
links_inside_skill and guardrails_for_risk.
...
# links_inside_skill is a line-based heuristic, not a markdown parser. It skips STATE.md (a
# decision log the running skill never loads), fenced code blocks, URLs, anchors, and
# placeholder/glob paths (<name>, {x}, [x], *, $VAR). Markdown link targets always count. An
# inline-code path counts only when a pointer phrase (see, follow, per, consult, refer to,
# defined/described/documented in, according to) sits at most three words before it and the
# line has no subject cue (if present, exists, project's, target, current repo). It misses bare
# prose mentions, pointers phrased any other way, and bare `docs/`/`references/` directory
# names; a nonexistent in-skill path is not reported (that is a broken link, not an escape).
OUTSIDE_FILE_NAMES = {"CLAUDE.md", "CLAUDE.global.md", "AGENTS.md"}
...
POINTER_BEFORE_RE = re.compile(
    r"(?i)\b(see|follow|follows|following|per|consult|refer to|defined in|described in|documented in|according to)\s+(?:[^\s`]+\s+){0,3}$"
)
SUBJECT_CUE_RE = re.compile(r"(?i)\b(if present|when present|exists?|project's|target|current repo)\b")


def _outside_reason(target: str, file_dir: str, skill_root: str) -> Optional[str]:
    target = target.split("#", 1)[0].rstrip("/")
    if not target or URL_RE.match(target) or PLACEHOLDER_RE.search(target):
        return None
    if target.startswith("~"):
        return "home-relative path"
    if target.startswith("/"):
        return "absolute path" if target.count("/") >= 2 else None

    resolved = os.path.normpath(os.path.join(file_dir, target))
    if resolved != skill_root and not resolved.startswith(skill_root + os.sep):
        return "resolves outside the skill folder"

    # Prose paths may be written relative to the file or to the skill root.
    exists_inside = os.path.exists(resolved) or os.path.exists(os.path.normpath(os.path.join(skill_root, target)))
    if exists_inside:
        return None
    if os.path.basename(target) in OUTSIDE_FILE_NAMES:
        return f"references {os.path.basename(target)}"
    segments = os.path.normpath(target).split(os.sep)
    first_segment = segments[0]
    if first_segment in {"docs", "references"} and len(segments) > 1:
        return f"{first_segment}/ path not inside the skill"
    return None


def find_outside_links(skill_root: str) -> list[str]:
    skill_root = os.path.normpath(os.path.abspath(skill_root))
    findings: list[str] = []
    for dirpath, _dirnames, filenames in os.walk(skill_root):
        for filename in sorted(filenames):
            if not filename.endswith(".md") or filename == "STATE.md":
                continue
            file_path = os.path.join(dirpath, filename)
            rel_file = os.path.relpath(file_path, skill_root)
            ...
            for line_no, line in enumerate(lines, start=1):
                ...
                targets = MD_LINK_RE.findall(prose) + REF_LINK_RE.findall(prose)
                if not SUBJECT_CUE_RE.search(prose):
                    targets += [
                        m.group(1).strip()
                        for m in INLINE_CODE_RE.finditer(line)
                        if " " not in m.group(1).strip() and POINTER_BEFORE_RE.search(line[: m.start()])
                    ]

                for target in targets:
                    reason = _outside_reason(target, dirpath, skill_root)
                    if reason:
                        findings.append(f"{rel_file}:{line_no} `{target}` ({reason})")
    return findings
...
def validate_skill(skill_path: str) -> dict:
    ...
    outside_links = find_outside_links(skill_path)
    add_check(
        "links_inside_skill",
        not outside_links,
        "No loaded or followed path escapes the skill folder"
        if not outside_links
        else f"{len(outside_links)} path(s) point outside the skill folder — state the rule inline or move the file into the skill: "
        + "; ".join(outside_links),
    )
    ...
    if risk is None or str(risk).strip() == "":
        add_check(
            "guardrails_for_risk",
            False,
            "Missing metadata.risk — record low, medium, or high",
            severity="warning",
        )
    else:
        risk_level = str(risk).strip().lower()
        if risk_level not in {"low", "medium", "high"}:
            add_check("guardrails_for_risk", False, f"metadata.risk '{risk}' must be low, medium, or high")
        elif risk_level == "low":
            add_check("guardrails_for_risk", True, "metadata.risk: low — no ## Guardrails section required")
        else:
            headings = re.findall(r"(?m)^##\s+(.+?)\s*$", _strip_fenced_blocks(body))
            guardrails_idx = next((i for i, h in enumerate(headings) if h.lower() == "guardrails"), None)
            workflow_idx = next(
                (i for i, h in enumerate(headings) if re.search(r"(?i)instructions|workflow|step|phase", h)),
                None,
            )
            if guardrails_idx is None:
                add_check("guardrails_for_risk", False, f"metadata.risk: {risk_level} requires a ## Guardrails section in SKILL.md")
            elif workflow_idx is not None and guardrails_idx > workflow_idx:
                add_check(
                    "guardrails_for_risk",
                    False,
                    f"## Guardrails must come before the first workflow heading '## {headings[workflow_idx]}'",
                )
            else:
                add_check("guardrails_for_risk", True, f"metadata.risk: {risk_level} — ## Guardrails precedes the workflow")
